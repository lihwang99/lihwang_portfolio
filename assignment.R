...
has_more_zs <- function(string_a, string_b) {
  a_string <- tolower(string_a)
  b_string <- tolower(string_b)
  string_a_zs <- str_count(a_string, pattern = "z")
  string_b_zs <- str_count(b_string, pattern = "z")
  print(string_a_zs)
  print(string_b_zs)
  if(string_a_zs > string_b_zs) {
    more_z <- a_string
  }
  else if(string_b_zs > string_a_zs) {
    more_z <- b_string
  }
  else if(string_a_zs == 0 && string_b_zs == 0) {
    more_z <- "Neither string contains the letter z."
  }
  else if(string_a_zs == string_b_zs) {
    more_z <- "The strings have the same number of Zs."
  }
  return(more_z)
}
...
more_zs <- has_more_zs("Aziz", "zebra")
...
# - `mean`: in which you'll
# store the mean of the vector
...
# Now, write a function called `summarize_vector`
# that takes in a vector of numbers,
# and returns a list of summary information about that vector
# (including the mean, median, and length)
summarize_vector <- function(number_vector) {
list(mean = mean(number_vector), median = median(number_vector),
     length = length(number_vector))
}
...
# Create a variable `num_courses` that counts
# the number of courses stored
